...
/* ─────────────────────────────────────────────────────────
   PowerPoint Export — one slide per section
───────────────────────────────────────────────────────── */
export async function exportDashboardPPTX(dashboardData) {
    try {
        const pptx = new PptxGenJS();
        pptx.layout = 'LAYOUT_16x9';
        pptx.title = 'TVS-PED Factory Command Centre';
        ...
        cover.background = { color: '060D1F' };
        cover.addText('Factory Command Centre', {
            x: 0.8, y: 1.5, w: 8.4, h: 1.2,
            fontSize: 36, bold: true, color: WHITE, fontFace: 'Inter',
        });
        cover.addText('TVS Motor Company — PED Analytics Dashboard', {
            x: 0.8, y: 2.8, w: 8.4, h: 0.5,
            fontSize: 16, color: '7B8AAB', fontFace: 'Inter',
        });
        cover.addText(`Generated: ${new Date().toLocaleDateString('en-IN', { dateStyle: 'long' })}`, {
            x: 0.8, y: 3.5, w: 8.4, h: 0.4,
            fontSize: 12, color: '7B8AAB', fontFace: 'Inter',
        });
        ...
        if (dashboardData?.mhSummary) {
            ...
            kpiSlide.background = { color: 'F5F7FA' };
            kpiSlide.addText('Key Performance Indicators', {
                x: 0.5, y: 0.3, w: 9, h: 0.5,
                fontSize: 20, bold: true, color: BLUE, fontFace: 'Inter',
            });
            const kpis = [
                { label: 'Total Requested', value: s.totalRequested, col: '253C80' },
                { label: 'Approved',        value: s.totalApproved,  col: GREEN  },
                { label: 'Pending',         value: s.totalPending,   col: AMBER  },
                { label: 'Rejected',        value: s.totalRejected,  col: RED    },
                { label: 'Approval Rate',   value: `${s.approvalRate}%`, col: GREEN },
                { label: 'Emails Sent',     value: t.emailsDispatched, col: '378ADD' },
                { label: 'PED Engineers',   value: t.activePEDEngineers, col: '534AB7' },
                { label: 'Active Vendors',  value: t.activeVendors,  col: '0F6E56' },
            ];
            kpis.forEach((k, i) => {
                const x = 0.5 + (i % 4) * 2.25;
                const y = 1.0 + Math.floor(i / 4) * 1.6;
                kpiSlide.addShape(pptx.ShapeType.roundRect, { x, y, w: 2.1, h: 1.4, rectRadius: 0.15, fill: { color: WHITE }, line: { color: k.col, width: 2 } });
                kpiSlide.addText(String(k.value ?? 0), { x, y: y + 0.25, w: 2.1, h: 0.6, fontSize: 24, bold: true, color: k.col, align: 'center', fontFace: 'Inter' });
                kpiSlide.addText(k.label, { x, y: y + 0.85, w: 2.1, h: 0.4, fontSize: 10, color: '7B8AAB', align: 'center', fontFace: 'Inter' });
            });
        }

        // ── Slide 3: MH Trend Table ─────────────────────────────────────────
        if (dashboardData?.mhTrend?.length) {
            const trendSlide = pptx.addSlide();
            trendSlide.background = { color: 'F5F7FA' };
            trendSlide.addText('MH Request Trend — Last 12 Months', {
                x: 0.5, y: 0.3, w: 9, h: 0.5,
                fontSize: 20, bold: true, color: BLUE, fontFace: 'Inter',
            });
            const rows = [
                [{ text: 'Month',     options: { bold: true, color: WHITE, fill: BLUE } },
                 { text: 'Requested', options: { bold: true, color: WHITE, fill: BLUE } },
                 { text: 'Approved',  options: { bold: true, color: WHITE, fill: BLUE } },
                 { text: 'Rejected',  options: { bold: true, color: WHITE, fill: BLUE } }],
                ...dashboardData.mhTrend.map(m => [
                    { text: m.month },
                    { text: String(m.requested) },
                    { text: String(m.approved), options: { color: GREEN } },
                    { text: String(m.rejected), options: { color: RED   } },
                ])
            ];
            trendSlide.addTable(rows, { x: 0.5, y: 1.0, w: 9, fontSize: 10, fontFace: 'Inter', border: { color: 'E0E4EF', pt: 0.5 }, rowH: 0.28 });
        }

        // ── Slide 4: Vendor Performance ─────────────────────────────────────
        if (dashboardData?.vendorPerformance?.length) {
            const vendorSlide = pptx.addSlide();
            vendorSlide.background = { color: 'F5F7FA' };
            vendorSlide.addText('Vendor Performance', {
                x: 0.5, y: 0.3, w: 9, h: 0.5,
                fontSize: 20, bold: true, color: BLUE, fontFace: 'Inter',
            });
            const rows = [
                [{ text: 'Vendor',     options: { bold: true, color: WHITE, fill: BLUE } },
                 { text: 'Score /100', options: { bold: true, color: WHITE, fill: BLUE } },
                 { text: 'On-Time %',  options: { bold: true, color: WHITE, fill: BLUE } },
                 { text: 'Delay %',    options: { bold: true, color: WHITE, fill: BLUE } }],
                ...dashboardData.vendorPerformance.map(v => [
                    { text: v.vendorName },
                    { text: String(v.avgScore), options: { color: v.avgScore >= 75 ? GREEN : v.avgScore >= 50 ? AMBER : RED } },
                    { text: `${v.onTimeRate}%` },
                    { text: `${v.defectRate}%` },
                ])
            ];
            vendorSlide.addTable(rows, { x: 0.5, y: 1.0, w: 9, fontSize: 11, fontFace: 'Inter', border: { color: 'E0E4EF', pt: 0.5 }, rowH: 0.32 });
        }

        await pptx.writeFile({ fileName: `TVS-PED-Dashboard-${new Date().toISOString().slice(0, 10)}.pptx` });
    } catch (err) {
